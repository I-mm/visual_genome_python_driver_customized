...
class Object:
    ...
    def __init__(self, object_id, x, y, w, h, names, synsets, image):
        self.id = object_id
        self.x = x
        self.y = y
        self.w = w
    ...
    def __str__(self):
        ...
        stat_str = 'object_id: {0}, x: {1}, y: {2}, w: {3},' \
                   'h: {4}, names: {5}, synsets: {6}, image: {7}'
        # return stat_str.format(self.id, self.x, self.y,
        #                        self.w, self.h, self.names[0] if len(self.names) != 0 else 'None', self.synsets,
        #                        self.image.id)
        return stat_str.format(self.id, self.x, self.y,
                               self.w, self.h, self.names, self.synsets,
                               self.image.id)
    def __repr__(self):
        return str(self)
...
class Relationship:
    ...
    def __init__(self, id, subject, predicate, object, synset, image):
        self.id = id
    ...
    def __str__(self):
        return "{0}: {1} {2} {3} ({4})".format(self.id, self.subject,
                                         self.predicate, self.object, self.image.id)
