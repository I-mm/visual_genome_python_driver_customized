...
def get_objects_of_image(info_objects_json):
    """
    Get object data
    """
    return utils.parse_objects(info_objects_json['objects'], get_image_data(info_objects_json['image_id']))

def get_objects_of_image_byimageid(objects_json, image_id):
    """
    Get object data
    """
    info_objects_json = []
    for item in objects_json:
        if item['image_id'] == image_id:
            info_objects_json = item
            break
    if len(info_objects_json) == 0:
        return []

    return utils.parse_objects(info_objects_json['objects'], get_image_data(info_objects_json['image_id']))

def get_relationships_of_image(info_relationships_json):
    """
    Get relationships data
    """
    return utils.parse_relationships(info_relationships_json['relationships'], get_image_data(info_relationships_json['image_id']))

def get_relationships_of_image_byimageid(relationships_json, image_id):
    """
    Get relationships data
    """
    info_relationships_json = []
    for item in relationships_json:
        if item['image_id'] == image_id:
            info_relationships_json = item
            break
    if len(info_relationships_json) == 0:
        return []

    return utils.parse_relationships(info_relationships_json['relationships'], get_image_data(info_relationships_json['image_id']))

def get_attributes_of_image(info_attributes_json):
    """
    Get of all attributes in an image.
    """
    return utils.parse_attributes(info_attributes_json['attributes'], get_image_data(info_attributes_json['image_id']))

def get_attributes_of_image_byimageid(attributes_json, image_id):
    """
    Get of all attributes in an image.
    """
    info_attributes_json = []
    for item in attributes_json:
        if item['image_id'] == image_id:
            info_attributes_json = item
            break
    if len(info_attributes_json) == 0:
        return []

    return utils.parse_attributes(info_attributes_json['attributes'], get_image_data(info_attributes_json['image_id']))

def get_region_descriptions_of_image_byid(id=61512):
    """
...
def get_region_descriptions_of_image_byimage(image):
    """
    Get the region descriptions of an image.
    """
    data = utils.retrieve_data('/api/v0/images/' + str(image.id) + '/regions')
    if 'detail' in data and data['detail'] == 'Not found.':
        return None
    return utils.parse_region_descriptions(data, image)
